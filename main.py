...
def start_game(n: int = 0):
    ...
    cur.execute("""
        SELECT 
            ...
            (SELECT STRING_AGG(s.species, ', ') FROM dccomicsdle_schema.species s WHERE s.charname = c.charname) AS species,
            (SELECT STRING_AGG(p.powers, ', ') FROM dccomicsdle_schema.powers p WHERE p.charname = c.charname) AS powers,
            (SELECT STRING_AGG(a.affiliations, ', ') FROM dccomicsdle_schema.affiliations a WHERE a.charname = c.charname) AS affiliations,
            (SELECT STRING_AGG(ap.apperances, ', ') FROM dccomicsdle_schema.appearance_types ap WHERE ap.charname = c.charname) AS appearances
